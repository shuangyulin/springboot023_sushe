<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
		    ...
		    label="床位">
		     <template slot-scope="scope">
                ...
                label="操作">
                <template slot-scope="scope">
                                                                <el-button v-if="isAuth('xueshengxinxi','日常事物评分') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="richangshiwuCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'日常事物评分':'' }}</el-button>
                <el-button v-if="isAuth('xueshengxinxi','日常事物评分') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="richangshiwuCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'日常事物评分':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xueshengxinxi','日常事物评分') && contents.tableBtnIcon == 0" type="success" size="mini" @click="richangshiwuCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'日常事物评分':'' }}</el-button>
                                                <el-button v-if="isAuth('xueshengxinxi','申请床位') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="chuangweianpaiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'申请床位':'' }}</el-button>
                <el-button v-if="isAuth('xueshengxinxi','申请床位') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="chuangweianpaiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'申请床位':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xueshengxinxi','申请床位') && contents.tableBtnIcon == 0" type="success" size="mini" @click="chuangweianpaiCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'申请床位':'' }}</el-button>
                                                <el-button v-if="isAuth('xueshengxinxi','卫生记录') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="qingligonggongchangsuoCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'卫生记录':'' }}</el-button>
                <el-button v-if="isAuth('xueshengxinxi','卫生记录') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="qingligonggongchangsuoCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'卫生记录':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xueshengxinxi','卫生记录') && contents.tableBtnIcon == 0" type="success" size="mini" @click="qingligonggongchangsuoCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'卫生记录':'' }}</el-button>
    ...
    <add-or-update v-if="addOrUpdateFlag" :parent="this" ref="addOrUpdate"></add-or-update>
            ...
            <richangshiwu-cross-add-or-update v-if="richangshiwuCrossAddOrUpdateFlag" :parent="this" ref="richangshiwuCrossaddOrUpdate"></richangshiwu-cross-add-or-update>
        <chuangweianpai-cross-add-or-update v-if="chuangweianpaiCrossAddOrUpdateFlag" :parent="this" ref="chuangweianpaiCrossaddOrUpdate"></chuangweianpai-cross-add-or-update>
        <qingligonggongchangsuo-cross-add-or-update v-if="qingligonggongchangsuoCrossAddOrUpdateFlag" :parent="this" ref="qingligonggongchangsuoCrossaddOrUpdate"></qingligonggongchangsuo-cross-add-or-update>
...
import richangshiwuCrossAddOrUpdate from "../richangshiwu/add-or-update";
import chuangweianpaiCrossAddOrUpdate from "../chuangweianpai/add-or-update";
import qingligonggongchangsuoCrossAddOrUpdate from "../qingligonggongchangsuo/add-or-update";
export default {
  data() {
    return {
                                                                                                                                                searchForm: {
      ...
      addOrUpdateFlag:false,
                  richangshiwuCrossAddOrUpdateFlag: false,
            chuangweianpaiCrossAddOrUpdateFlag: false,
            qingligonggongchangsuoCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
            richangshiwuCrossAddOrUpdate,
        chuangweianpaiCrossAddOrUpdate,
        qingligonggongchangsuoCrossAddOrUpdate,
          },
  methods: {
    contentStyleChange() {
    ...
    contentPageStyleChange(){
      ...
      this.contents.pageEachNum = 10
            ...
            richangshiwuCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.richangshiwuCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xueshengxinxi');
      this.$nextTick(() => {
        this.$refs.richangshiwuCrossaddOrUpdate.init(row.id,type);
      });
    },
        chuangweianpaiCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.chuangweianpaiCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xueshengxinxi');
      this.$nextTick(() => {
        this.$refs.chuangweianpaiCrossaddOrUpdate.init(row.id,type);
      });
    },
        qingligonggongchangsuoCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.qingligonggongchangsuoCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','xueshengxinxi');
      this.$nextTick(() => {
        this.$refs.qingligonggongchangsuoCrossaddOrUpdate.init(row.id,type);
      });
    },
                    init () {
                                                                                                                                                                                                        },
